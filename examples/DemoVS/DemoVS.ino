...
#include "../../Commanders/VStudio/Commanders.h"
#else
#include <LiquidCrystal.h>
...
LiquidCrystal lcd;
#else
LiquidCrystal lcd(0, 4, 5, 6, 7, 3);  // Set the LCD I2C address
#endif
LcdUi lcdui;
ScreenLiquid screen;
...
void setup()
{
	...
	Screen::NoMsg = STR_NO;
	screen.HeaderY = 2;
	screen.SecondLineY = 3;
	lcdui.Setup(&screen, 11);

	lcdui.AddWindow(new WindowSplash(STR_SPLASH1, STR_SPLASH2, 500));	// Splash screen
	WindowChoice *pChoiceMain = (WindowChoice *)lcdui.AddWindow(new WindowChoice(STR_MODEMODECHOICE, 3, false));	// menu
	pChoiceMain->AddChoice(STR_MODECONFIG);
	WindowChoice *pChoiceConfig = (WindowChoice *)lcdui.AddWindow(new WindowChoice(STR_MODECONFIG, 4, false), pChoiceMain, 0);	// config
	pChoiceConfig->AddChoice(STR_INCCFG);
	lcdui.AddWindow(new WindowInt(STR_INCCFG), pChoiceConfig, 0);	// config incValue
	pChoiceConfig->AddChoice(STR_NAMECFG);
	lcdui.AddWindow(new WindowText(STR_NAMECFG, 10), pChoiceConfig, 1);	// config name
	pChoiceConfig->AddChoice(STR_BACKLIGHTCFG);
	lcdui.AddWindow(new WindowYesNo(STR_BACKLIGHTCFG), pChoiceConfig, 2);	// config backlight
	...
	lcdui.AddWindow(new WindowChooseDcFreq(STR_FREQ), pChoiceMain, 2); // vhange freq
	...
	buttons.Setup();
#ifdef VISUALSTUDIO
	ButtonsCommanderKeyboard *pButMore = new ButtonsCommanderKeyboard(EVENT_MORE);
	pButMore->Setup('+');
	buttons.Add(pButMore);
	ButtonsCommanderKeyboard *pButLess = new ButtonsCommanderKeyboard(EVENT_LESS);
	pButLess->Setup('-');
	buttons.Add(pButLess);
	ButtonsCommanderKeyboard *pButCancel = new ButtonsCommanderKeyboard(EVENT_CANCEL);
	pButCancel->Setup('/');
	buttons.Add(pButCancel);
	ButtonsCommanderKeyboard *pButSelect = new ButtonsCommanderKeyboard(EVENT_SELECT);
	pButSelect->Setup('*');
	buttons.Add(pButSelect);
#else
	// Buttons on a 'LCD Keypad shield', handled in analog way.
	ButtonsCommanderButton *pPushes = buttons.Add(new ButtonsCommanderAnalogPushes(5));
	...
	((ButtonsCommanderAnalogPushes*)pPushes)->Setup(A0, ids, values, 50);
#endif
}
	...
	if (lcdui.Loop(event))
	{
		...
		case STATE_INITIALIZE:
			...
			case STR_BACKLIGHTCFG:
				lcdui.SetChoiceValue(backlight ? Screen::YesMsg : Screen::NoMsg);
				break;
			case STR_FREQ:
				((WindowChooseDcFreq*)pCurrent)->SetCurrentChoice(freq);
				break;
			}
			break;
		...
		case STATE_CONFIRMED:
			...
			case STR_NAMECFG:
				...
				break;
			case STR_BACKLIGHTCFG:
				backlight = lcdui.GetChoiceValue() == Screen::YesMsg;
				break;
			case STR_FREQ:
				freq = ((WindowChooseDcFreq *)pCurrent)->GetChoiceValue();
				break;
			}
